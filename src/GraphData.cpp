...
void GD::Enumerate(TNGraph &G, long unsigned int k, GD::GraphList *kSubgraphs) {
	...
	std::vector<std::vector<long unsigned int>> Neighbors(G.GetNodes());
	...
	for(long unsigned int i=0; G.IsNode(i); i++) { 								// iterate through all vertices of G
		#ifdef DEBUG
			if(DEBUG_LEVEL>=2) printf("Root: %lu\n", i);
		#endif
...
void GD::Explore(TNGraph &G, std::vector<std::vector<long unsigned int>> &Neighbors, long unsigned int root, std::vector<bool> &Visited, std::vector<long unsigned int> &S, std::vector<long unsigned int> &subgraph, GD::GraphList *kSubgraphs, long unsigned int Remainder) {
	#ifdef DEBUG
		LEVEL++;
	#endif
	uint64 t0 = GetTimeMs64();
					 ...
					 GD::GraphList *kSubgraphs, std::string destination)
{
	for(int m=0; m<Motifs.size(); m++) {
		...
		std::stringstream ss;
		ss << destination << "motif_ID" << IDs[m] << "_highlighted.gdf";
		std::string s = ss.str();
		const char* path = s.c_str();
		std::ofstream GDF;
		GDF.open(path);
